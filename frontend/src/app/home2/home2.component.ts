import { Component, OnInit, ViewChild, ElementRef  } from '@angular/core';
import { Router } from '@angular/router';
import { HttpClient } from '@angular/common/http';
import { FormBuilder, FormGroup, Validators } from '@angular/forms';
import { DataSource } from '@angular/cdk/collections';
import {HomeService} from '../service/home.service';
import { MatIconRegistry } from "@angular/material/icon";
import { DomSanitizer } from "@angular/platform-browser";
import {ClassService } from '../service/class.service';
import {ThemePalette} from '@angular/material/core';

export interface userLogin {
  username: string;
  password: string;
}

export interface ChipColor {
  name: string;
  color: ThemePalette;
}

export interface classOrder {
  subject_id : number;
  TSName : string;
  TSDescription : string;
  TSID : string;
  TSTeacher : string;
  TStimesubject : Array<newTimeSubject>;
}

export interface newTimeSubject {
  Time : string;
}

export interface timeSubjectAuthen {
  Time : string;
  TimeoutTemp : string;
}

@Component({
  selector: 'app-home2',
  templateUrl: './home2.component.html',
  styleUrls: ['./home2.component.css']
})

export class Home2Component implements OnInit {

  @ViewChild('formAddTimeSubject', {static: false}) dataContainer: ElementRef;

  private mypic: string = "data:image/gif;base64,";
  private teacherClass: object = null;
  private userdata = null;
  private loginas : string = null;
  private loginReply : any;
  private reader = new FileReader();
  public getPic = null; 
  hide = false;
  private joinName : string = null;
  private Username : string = null;
  private arr : any = [];
  private arrdata : any = [];
  public countTimeSubject = 2;
  private startAuthen : string = null;
  private lastAuthen : string = null;
  public newAddTime : any[] = [];
  private testArray : Array<newTimeSubject> = [];

  constructor(
    private homeService : HomeService,private matIconRegistry: MatIconRegistry,
    private domSanitizer: DomSanitizer,private router : Router,
    private classservice : ClassService,)
    { 
    this.matIconRegistry.addSvgIcon(
      "add",
      this.domSanitizer.bypassSecurityTrustResourceUrl("../../assets/add24px.svg")
    );
    this.matIconRegistry.addSvgIcon(
      "delete",
      this.domSanitizer.bypassSecurityTrustResourceUrl("../../assets/delete24px.svg")
    );
    this.matIconRegistry.addSvgIcon(
      "edit",
      this.domSanitizer.bypassSecurityTrustResourceUrl("../../assets/create-24px.svg")
    );
    this.matIconRegistry.addSvgIcon(
      "alarm",
      this.domSanitizer.bypassSecurityTrustResourceUrl("../../assets/alarm-24px.svg")
    );
  }

  userlogins: userLogin = {
    username: null,
    password: null,
  };

  newTime : newTimeSubject = {
    Time : null,
  }

  newclass : classOrder = {
    subject_id : null,
    TSName: null,
    TSDescription: null,
    TSID: null,
    TSTeacher: null,
    TStimesubject: null,
  };

  select_class : classOrder = {
    subject_id : null,
    TSName: null,
    TSDescription: null,
    TSID: null,
    TSTeacher: null,
    TStimesubject: null,
  };

  newtimeSubjectAuthen : timeSubjectAuthen = {
    Time : null,
    TimeoutTemp : null,
  }

  ngOnInit() {
    // this.homeService.getStatus().subscribe(
    //   data => {
    //     var statusBackend = data;
    //     if (statusBackend == "online"){
    //       this.homeService.status(statusBackend);
    //     }
    //   } ,
    //   error => {
    //     this.logout();
    //   }
    // );
    this.loadData();
    var show_ui = document.getElementById("mainId");
    var stat_login = localStorage.getItem('stateLogin');
    var state_btn = true;
    var btn_creat = document.getElementById("bt_createClass");
    var create = document.getElementById("create");
    var panel_create = document.getElementById("CreateClass");
    var img;
    var btnJoinClass = document.getElementById("btJoinClass");
    var panelJoinClass = document.getElementById("joinContainer");
    var edit_AlarmContainer = document.getElementById("editAlarmContainer"); 
    this.Username = localStorage.getItem('isLogin');

    if (stat_login == 'true'){
      show_ui.style.display = "none";
    }

    btn_creat.onclick = function(){
      panel_create.style.display = "flex";
    }

    btnJoinClass.onclick = function(){
      panelJoinClass.style.display = "flex";
    }

    window.onclick = function(event) {
      if (event.target == panel_create) {
        panel_create.style.display = "none";
      }
      if (event.target == panelJoinClass) {
        panelJoinClass.style.display = "none";
      }
      if (event.target == edit_AlarmContainer) {
        edit_AlarmContainer.style.display = "none"
      }
    }
  }

  checkLogin(){
    if(this.userlogins.username == null){
      alert("กรุณาป้อน User name");
    }else if(this.userlogins.password == null){
      alert("กรุณาป้อน Password");
    }else{
      this.login();
    }
  }
  
  login(){
    var show_ui = document.getElementById("mainId");
    this.homeService.LoginUser(this.userlogins).subscribe(
      data => {
        this.loginReply = data;
        if(this.loginReply == "Success"){
          this.homeService.login(this.userlogins.username, 'true');
          show_ui.style.display = "none";
          this.loadData();
        }
        else {
          alert("Username และ Password ของคุณไม่ถูกต้องกรุณาล็อคอินใหม่")
        }
      },
      error => {
        alert("server ERROR " + error);
    }
    );
  }

  logout(){
    localStorage.clear();
    window.location.reload();

  }

  checkClassOrder(){
    if(this.newclass.TSName == null){
      alert('กรุณาป้อนชื่อคลาสเรียนด้วยครับ')
    }else if(this.newclass.TSID == null){
      alert('กรุณาระบุคลาสไอดีด้วยครับ')
    }else{
      (<HTMLInputElement>document.getElementById('clear_class1')).disabled = true;
      (<HTMLInputElement>document.getElementById('clear_class2')).disabled = true;
      (<HTMLInputElement>document.getElementById('clear_class3')).disabled = true;
      (<HTMLInputElement>document.getElementById('bt_saveClass')).disabled = true;
      this.setUser();
      this.classSave();
    }
  }
  setUser(){
    this.newclass.TSTeacher = localStorage.getItem('id');
    if(this.newclass.TSTeacher == null){
    }
  }
  classSave(){
    var panel_create = document.getElementById("CreateClass");
    this.homeService.CreateClass(this.newclass).subscribe(
      data =>{
      var checkOK;
      checkOK = data;
      if(checkOK == "Success"){
        panel_create.style.display = "none";
        (<HTMLInputElement>document.getElementById('clear_class1')).value = ' ';
        (<HTMLInputElement>document.getElementById('clear_class2')).value = ' ';
        (<HTMLInputElement>document.getElementById('clear_class3')).value = ' ';
        (<HTMLInputElement>document.getElementById('clear_class1')).disabled = false;
        (<HTMLInputElement>document.getElementById('clear_class2')).disabled = false;
        (<HTMLInputElement>document.getElementById('clear_class3')).disabled = false;
        (<HTMLInputElement>document.getElementById('bt_saveClass')).disabled = false;
        this.loadData();
      }
    },
    error => {
      alert('Error กรุณาป้อนข้อมูลใหม่อีกครั้งครับ' + error);
    }
    );
  }

  loadData() {
    this.homeService.getUserdata().subscribe(data =>{  //get ID
      this.userdata = data; //มาแค่ID
      // this.homeService.setID(this.userdata); //สำหรับserver Test
      this.homeService.setID(this.userdata[0]['user_id']); //สำหรับserver DB
      this.homeService.getGetPic().subscribe(data =>{ //get Picture
        this.getPic = data;
        if (this.getPic.trim() === ''){}
        else {  this.mypic = this.getPic }
      });
      this.homeService.getClass().subscribe(data =>{ //get My class
        this.teacherClass = data;
      });
    });
    this.Username = localStorage.getItem('isLogin');
  }

  selectClass(x){
    this.classservice.classSelect(x.subject_id);
    this.router.navigate(['/Class']);
  }  

  isEmptyOrSpaces(str){
    return str === null || str.match(/^ *$/) !== null;
  }

  join(){
    var panel = document.getElementById("joinContainer");
    var x = this.joinName;
    panel.style.display = "none";
    (<HTMLInputElement>document.getElementById('inputJoinClass')).value = ' ';
  }

  randomImg(index,i){
    var backgroungImg = ['../../assets/almond-blossom-5378_1920-min.jpg', 
    '../../assets/books-1245690_1920-min.jpg', 
    '../../assets/cereals-100263_1920-min.jpg', 
    '../../assets/cherry-blossom-1209577_1920-min.jpg', 
    '../../assets/cherry-tree-1225186_1920-min.jpg',
    '../../assets/evening-567840_1920-min.jpg', 
    '../../assets/insects-4489864_1920-min.jpg',
    '../../assets/japanese-cherry-trees-324175_1920-min.jpg', 
    '../../assets/knowledge-1052011_1920-min.jpg',
    '../../assets/macbook-336704_1920-min.jpg', 
    '../../assets/melons-4507974_1920-min.jpg', 
    '../../assets/notebook-336634_1920-min.jpg',
    '../../assets/old-books-436498_1920-min.jpg', 
    '../../assets/spring-bird-2295434_1920-min.jpg', 
    '../../assets/sunset-174276_1920-min.jpg',
    '../../assets/sunset-1373171_1920-min.jpg', 
    '../../assets/tree-736881_1280-min.jpg', 
    '../../assets/tree-838667_1280-min.jpg',
    '../../assets/typewriter-801921_1920-min.jpg']
    var ranImg = backgroungImg[Math.floor(Math.random()*backgroungImg.length)];
    if (this.arrdata.length < 1 || this.arrdata == undefined){
      this.arrdata.push(index.TSName);
    }
    if (this.arrdata[i] != index.TSName){
      this.arrdata.push(index.TSName);
    }
    if (this.arr.length < this.arrdata.length){
      this.arr.push(ranImg);
    }
    return this.arr
  }

  clickEditAlarm(x){
    this.classservice.classSelect(x.subject_id);
    this.classservice.getmyClass().subscribe(data =>{
      var myClass = data;
      localStorage.setItem('passOfCouse', myClass[0]["TSpassword"]);
    });
    var edit_AlarmContainer = document.getElementById("editAlarmContainer");
    edit_AlarmContainer.style.display = "flex";
  }

  private addTimeSubject(){
    console.log("Hello Add time subject!")
    var startTime = this.startAuthen;
    var lastTime = this.lastAuthen;
    var timeSubject = startTime + "-" + lastTime;
    this.newTime.Time = timeSubject;
    this.newAddTime.push({Time : timeSubject});
    this.newclass.TStimesubject = this.newAddTime;
    console.log(this.newclass);
  }

  removeInput(index) {
    this.newAddTime.splice(index,1)
    this.newclass.TStimesubject = this.newAddTime;
    console.log(this.newclass);
  }

  private UpdateTime(){
    this.homeService.updateTime(this.newtimeSubjectAuthen).subscribe(data => {
      var check = data;
      if (check == "Success"){
        (<HTMLInputElement>document.getElementById('input1')).value = ' ';
        (<HTMLInputElement>document.getElementById('input2')).value = ' ';
        (<HTMLInputElement>document.getElementById('editAlarmContainer')).style.display = "none";
      }
    })
  }
}

